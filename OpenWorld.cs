...
namespace XenoLib
{
    ...
    public class OpenWorldCell
    {
        ...
        /// Saves data
        /// </summary>
        /// <param name="sw">File stream to save to</param>
        /// <param name="saveAuto">Close fileStream when done flag value</param>
        public virtual void saveData(StreamWriter sw, bool saveAuto = false)
        {
            sw.WriteLine("======OpenWorldCell Data======");
            sw.WriteLine(name);
            sw.WriteLine(cellx);
        ...
        public void addTunnel(int x1, int y1, int x2, int y2, int radius)
        {
            ...
            for(int r = 0; r < range; r++)
            {
                ...
                tx = (int)((Math.Cos(Helpers<int>.degreesToRadians(angle))) * radius) + tx;
                ty = (int)((Math.Sin(Helpers<int>.degreesToRadians(angle))) * radius) + ty;
    ...
    /// <summary>
    /// OpenWorldCell2 class
    /// </summary>
    public class OpenWorldCell2
    {
        //protected 
        protected XenoTileSys2 tiles;
        protected AutoTileSys autoTiles;
        protected SectorGraph64 sg;
        protected string name;
        protected Texture2D source;
        ...
        protected MapGraph localMG;
        protected List<string> localScripts;
        protected OpenWorld2 world;

        //public
        /// <summary>
        /// OpenWorldCell2 constructor
        /// </summary>
        /// <param name="source">Texture2D reference</param>
        /// <param name="autoSource">Texture2D reference</param>
        /// <param name="width">Width in tiles</param>
        ...
        /// <param name="sourceH">Source height in tiles</param>
        /// <param name="world">OpenWorld reference</param>
        public OpenWorldCell2(Texture2D source, Texture2D autoSource, int width, int height, int tileWidth,
            int tileHeight, int winWidth, int winHeight, int winx, int winy, int cellx, int celly,
            string name, bool spawnable = true, bool fill = true, bool randomize = false, int sourceW = 5,
            int sourceH = 7, OpenWorld2 world = null)
        {
            this.source = source;
            tiles = new XenoTileSys2(source, width, height, winWidth, winHeight, winx, winy, tileWidth, tileHeight, fill, randomize, sourceW, sourceH);
            autoTiles = new AutoTileSys(autoSource, width, height, tileWidth, tileHeight);
            sg = new SectorGraph64(width * tileWidth, height * tileHeight);
            this.name = name;
            this.cellx = cellx;
        ...
        /// </summary>
        /// <param name="obj">OpenWorldCell reference</param>
        public OpenWorldCell2(OpenWorldCell2 obj)
        {
            this.source = obj.Source;
            tiles = new XenoTileSys2(obj.Tiles);
            autoTiles = new AutoTileSys(obj.AutoTiles);
            sg = new SectorGraph64(obj.SG);
            this.name = obj.Name;
            this.cellx = obj.cellx;
        ...
        /// <summary>
        /// OpenWorld2 from file constructor
        /// </summary>
        /// <param name="source">Texture2D reference</param>
        /// <param name="autoSource">Texture2D reference</param>
        /// <param name="winWidth">Window width in tiles</param>
        /// <param name="winHeight">Window height in tiles</param>
        /// <param name="sr">StreamReader reference</param>
        /// <param name="world">OpenWorld2 reference</param>
        public OpenWorldCell2(Texture2D source, Texture2D autoSource, int winWidth, int winHeight, StreamReader sr, OpenWorld2 world = null)
        {
            this.source = source;
            this.autoSource = autoSource;
            sr.ReadLine();//discard testing data
            ...
            worldy = Convert.ToInt32(sr.ReadLine());
            spawnable = Convert.ToBoolean(sr.ReadLine());
            tiles = new XenoTileSys2(source, winWidth, winHeight, sr);
            autoTiles = new AutoTileSys(autoSource, sr);
            sg = new SectorGraph64(tiles.Width * tiles.TileWidth, tiles.Height * tiles.TileHeight);
            isInteriorCell = Convert.ToBoolean(sr.ReadLine());
            sr.ReadLine();//discard testing data
        ...
        /// <param name="avatar">XenoSprite reference</param>
        /// <param name="world">OpenWorld reference</param>
        public void intersectPortal(XenoSprite avatar, OpenWorld2 world)
        {
            Rectangle tempBox = new Rectangle(avatar.HitBox.X - (cellx * Tiles.Width * tiles.TileWidth), avatar.HitBox.Y - (celly * Tiles.Height * tiles.TileHeight), avatar.HitBox.Width, avatar.HitBox.Height);
            for (int p = 0; p < portals.Count; p++)
            {
    ...
    /// OpenWorld2 class
    /// </summary>
    public class OpenWorld2
    {
        //protected
        protected OpenWorldCell2 alpha;
        protected OpenWorldCell2 beta;
        protected OpenWorldCell2 delta;
        protected OpenWorldCell2 gamma;
        protected OpenWorldCell2 tempA;
        protected OpenWorldCell2 tempB;
        protected OpenWorldCell2 tempD;
        protected OpenWorldCell2 tempG;
        protected MapGraph mg;
        protected SimplePathFinder spf;
        protected List<XenoSprite> creatures;
        protected List<XenoSprite> npcs;
        ...
        /// <param name="planet">Planet name</param>
        /// <param name="isPlanet">IsPlanet flag value</param>
        public OpenWorld2(Texture2D source, Texture2D autoSource, int cellWidth, int cellHeight, int winx, int winy,
            int winWidth, int winHeight, int tileWidth, int tileHeight, int worldWidth, int worldHeight, int inputDelay = 1,
            string planet = "", bool isPlanet = false)
        {
            alpha = null;
        ...
        /// </summary>
        /// <param name="obj">OpenWorld reference</param>
        public OpenWorld2(OpenWorld2 obj)
        {
            alpha = obj.Alpha;
            beta = obj.Beta;
            delta = obj.Delta;
        ...
        /// <param name="cy">Celly value</param>
        /// <returns>OpenWorldCell</returns>
        public virtual OpenWorldCell2 getCellAt(int cx, int cy)
        {
            //check tempA, tempB, tempD and tempG for cell else try to load from file
            bool found = false;
            if (tempA != null)
        ...
        /// <param name="path">Folder path of cell to load</param>
        /// <returns>OpenWordCell</returns>
        public virtual OpenWorldCell2 loadCell(int cx, int cy, string path = "")
        {
            if (path == "")
            {
                path = AppDomain.CurrentDomain.BaseDirectory;
            ...
            OpenWorldCell2 cell = new OpenWorldCell2(source, autoSource, winWidth, winHeight, sr, this);
            sr.Close();
            //cell name is "cell_(cx value)_(cy value).owc"
            return cell;
        }
        ...
        /// <param name="path">Folder path for cell to load</param>
        /// <returns>OpenWorldCell</returns>
        public virtual OpenWorldCell2 loadInteriorCell(string name, string path = "")
        {
            if (path == "")
            {
                path = AppDomain.CurrentDomain.BaseDirectory;
            ...
            OpenWorldCell2 cell = new OpenWorldCell2(interSource, interAutoSource, winWidth, winHeight, sr);
            sr.Close();
            //cell name is "cell_(name).owc"
            cell.IsInteriorCell = true;
            interior = true;
        ...
        {
            string path = AppDomain.CurrentDomain.BaseDirectory;
            OpenWorldCell2 temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 0, 0, "cell_0_0");
            temp.Tiles.fillLayer(0, 0);
            alpha = temp;
            alpha.Tiles.setTile(99, 99, 96, 96);
            alpha.AutoTiles.addTile(97, 98);
            alpha.AutoTiles.addTile(96, 98);
            alpha.AutoTiles.addTile(96, 97);
            alpha.AutoTiles.addTile(96, 96);
            ...
            addPortalToCell(alpha, 93 * 32, 93 * 32, 0, 0, 50 * 32, 12 * 32, "dungeon", true);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 1, 0, "cell_1_0");
            temp.Tiles.fillLayer(0, 0);
            beta = temp;
            beta.Tiles.setTile(0, 99, 96, 96);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 1, 1, "cell_1_1");
            temp.Tiles.fillLayer(0, 0);
            delta = temp;
            delta.Tiles.setTile(0, 0, 96, 96);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 0, 1, "cell_0_1");
            temp.Tiles.fillLayer(0, 0);
            gamma = temp;
            gamma.Tiles.setTile(99, 0, 96, 96);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 2, 0, "cell_2_0");
            temp.Tiles.fillLayer(160, 32);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 2, 1, "cell_2_1");
            temp.Tiles.fillLayer(32, 160);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 2, 2, "cell_2_2");
            temp.Tiles.fillLayer(96, 160);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 2, 3, "cell_2_3");
            temp.Tiles.fillLayer(160, 96);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 0, 2, "cell_0_2");
            temp.Tiles.fillLayer(128, 128);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 0, 3, "cell_0_3");
            temp.Tiles.fillLayer(32, 32);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 1, 2, "cell_1_2");
            temp.Tiles.fillLayer(64, 64);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 1, 3, "cell_1_3");
            temp.Tiles.fillLayer(128, 96);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 3, 1, "cell_3_1");
            temp.Tiles.fillLayer(96, 128);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 3, 2, "cell_3_2");
            temp.Tiles.fillLayer(32, 96);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 3, 3, "cell_3_3");
            temp.Tiles.fillLayer(96, 64);
            temp.saveData(path, "", true);
            temp = new OpenWorldCell2(source, autoSource, CellWidth, cellHeight, tileWidth, tileHeight, winWidth, winHeight, 0, 0, 3, 0, "cell_3_0");
            temp.Tiles.fillLayer(64, 96);
            temp.saveData(path, "", true);
        }
        /// <summary>
        /// Generate a dungeon and save (doors and portals not yet incorporated)
        ...
        public virtual void generatedungeon(string savePath = "", string name = "dungeon", bool spawnable = true, bool saveAuto = true)
        {
            ...
            //generate a depth 3 dungeon for testing
            OpenWorldCell2 dungeon = new OpenWorldCell2(source, autoSource, cellWidth, cellHeight, tileWidth,
                tileHeight, winWidth, winHeight, 0, 0, 0, 0, name, spawnable);
            dungeon.Tiles.fillLayer(0, 0);
            dungeon.AutoTiles.fill();
            //add dungeon at 50, 10 to ensure space but add portal later
            dungeon.addChamber(6, 8, CHAMBER.DOWN, 50, 10, true);
            //add test portal back
        ...
        /// <param name="name">Target cell name</param>
        /// <param name="toInterior">Portal to an interior cell falg value</param>
        public void addPortalToCell(OpenWorldCell2 cell, float x, float y, int cellx, int celly, int tx, int ty, string name, bool toInterior)
        {
            if (cell != null)
            {
                cell.Portals.Add(new XenoPortal(TextureBank.getTexture("portal"), x, y, 64, 64, 2, cellx, celly, tx, ty, name, toInterior));
        ...
        /// <param name="y">Y value</param>
        /// <returns>OpenWorldCell</returns>
        public OpenWorldCell2 getCurrentCell(float x, float y)
        {
            if (alpha != null)
            {
                if (x >= alpha.CellLeftSide && x <= alpha.CellRightSide)
